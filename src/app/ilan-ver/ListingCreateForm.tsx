...
export default function ListingCreateForm({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    const loadProfileDefaults = async () => {
      ...
      const preferredName =
        data.user_type === "kurumsal"
          ? data.company_name || data.full_name
          : data.full_name;

      if (preferredName) {
        setValue("contact_name", preferredName, { shouldValidate: true });
      }
    };
  ...
  const onSubmit = async (values: ListingFormValues) => {
    ...
    if (uploadedImages.length === 0) {
      setUploadError("En az 1 fotoğraf yükleyin.");
      setCurrentStep(3);
      return;
    }
    ...
    const payload = {
      ...
      contact_name: preferredContactName || values.contact_name,
    ...
    if (error || !data) {
      setSubmitError(error?.message ?? "İlan kaydedilemedi.");
      return;
    }
  ...
  useEffect(() => {
    ...
    const supabase = createClient();
    fetchBrandsByCategory(supabase, selectedCategory).then(setFilteredBrands);
  }, [selectedCategory]);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-4xl px-4 py-4 lg:px-8">
            ...
            {/* ════ Adım 3 — Fiyat, Konum, İletişim ve Fotoğraf ════ */}
            {currentStep === 3 && (
              <section className="space-y-4">
                <h2 className="text-lg font-semibold">
                  Fiyat, Konum, İletişim ve Fotoğraf
                </h2>
                          ...
                          <button
                            ...
                            onClick={() =>
                              setUploadedImages((prev) =>
                                prev.filter(
                                  (item) =>
                                    item.public_id !== image.public_id,
                                ),
                              )
                            }
